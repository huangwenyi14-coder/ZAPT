...
|------|-----------|-----------------|-----------------|
| `process` | 4688, Sysmon 1, eCAR PROCESS | `process_name` | `command_line`, `process_ref`, `parent_ref`, `persistent`, `supplementary` (auto/none) |
| `file_collection` | eCAR FILE/READ for every selected file | `process_ref`, `files` | |
| `file_create` | Sysmon 11, eCAR FILE/CREATE | `process_ref`, `path` | `size_bytes` |
| `file_delete` | eCAR FILE/DELETE | `process_ref`, `path` | |
| `registry_query` | eCAR REGISTRY/READ | `process_ref`, `key` | `value_name` |
| `registry_set` | Sysmon 12/13, eCAR REGISTRY/MODIFY | `process_ref`, `key`, `value_name`, `value_data` | |
| `image_load` | Sysmon 7, eCAR MODULE/LOAD | `process_ref`, `image_loaded` | `signed`, `signature`, `signature_status` |
| `process_terminate` | 4689, Sysmon 5, eCAR PROCESS/TERMINATE | `process_ref` | |
| `archive_create` | eCAR FILE/READ for inputs, Sysmon 11 and eCAR FILE/CREATE for output | `process_ref`, `source_files`, `archive_path`, `size_bytes` | |
...
| `connection` | Zeek conn, eCAR FLOW, + web_access/zeek_http when `service: http` | `dst_ip` | `dst_port` (default 443), `hostname` (domain for DNS/SSL SNI), `service`, `source_ip`, `source_process_ref`, `source_file`, `terminate_source_process`, `method`, `uri`, `status_code`, `user_agent` |
...
| `scheduled_task_created` | 4698 | `task_name` | `task_content`, `source_process_ref`, `interval_minutes`（明确重复周期，分钟） |
| `log_cleared` | 1102 | | |
...
| `beacon` | Zeek conn/proxy/ASA (periodic connections) | `dst_ip`, `interval`, one of `end_time`/`duration`/`count` | `action` (allow/deny), `hostname`, `service`, `protocol`, `source_ip`, `source_process_ref`, `method`, `uri`, `user_agent`, `referrer`, `status_code`, `orig_bytes`, `resp_bytes`, `profile`, `http_sequence`, `jitter` (default: 0.15) |
...
Use `process_ref` when later actions must be owned by the same process. Set
`persistent: true` only when a later action reuses that process; a final
`connection` may set `terminate_source_process: true` to close its lifecycle
after the network connection ends. `file_collection` emits process-owned reads
for the files actually selected. `archive_create` reads its declared inputs,
creates `archive_path`, and records `size_bytes` as a host-local artifact. A
later `connection` with matching `source_file` inherits that exact size as its
originator payload and emits a final process-owned file read before the flow.
Generation fails if the process reference is not live, the artifact was not
created earlier on that host, or an explicit `orig_bytes` disagrees with the
artifact size.
...
报告导入编译器还会为事件写入可选的 `source_metadata`，用于记录事实编号、页码或
段落、字段来源、预期可见性以及合成必要性。该字段由报告流水线生成，普通手写场景可以省略。
其中 `source_fact` 表示完全来自报告，`partially_ai_completed` 表示含 AI 补全字段，
`partially_engine_generated` 表示含本地引擎安全补全字段，`fully_synthetic` 只用于建立
EvidenceForge 状态所必需且明确披露的合成桥接事件。
`registry_query` 和 `registry_set` 必须分开：前者仅表示读取，不得为了获得 Sysmon 12/13
而伪造注册表写入。

...
output/
  ...
  GOLD_Label.json                          # Compact private ATT&CK/record/storyline labels
  RECORD_GROUND_TRUTH.jsonl                # Instructor-only physical-record labels/provenance
...
`GOLD_Label.json` contains exactly three top-level sections. `ta_physical_ids`
groups malicious physical IDs under Enterprise ATT&CK v18.1 tactic numbers such
as `TA0002`; `physical_id_set` is the unique malicious-record union; and
`storyline_attack_steps` is a compact ordered mapping such as
`{"S1": ["pr-..."], "S2": ["pr-..."]}` with the physical IDs supporting each
step. It intentionally omits `relative_path`, `record_index`, and
`byte_offset`; those locators remain authoritative in
`RECORD_GROUND_TRUTH.jsonl` and the derived blind `RECORD_INDEX.jsonl`.

Generation validates that TA/step IDs belong to `physical_id_set`, every GOLD ID
resolves exactly once in the record-index source, and storyline step numbering is
ordered and contiguous from `S1`.

The participant-safe tactic ID/name catalog is published separately as
[`ATTACK_TACTICS_v18.1.json`](ATTACK_TACTICS_v18.1.json). It contains only
public ATT&CK metadata and no dataset or scoring answers.

`RECORD_GROUND_TRUTH.jsonl` contains one JSON object per final logical log
record without modifying the source-native logs. Each row includes a stable
`physical_record_id`, the generating `logical_event_id`, optional
`storyline_id`, `label` (`malicious`, `red_herring`, or `benign`), source
format/instance, relative file path, final record index, byte offset and length,
content hash, source-native identifiers, observed source time, and a
`correlation_features` object where available. The correlation object extracts
Windows/Sysmon process GUIDs, PIDs, logon/user fields, image/command/hash fields;
Zeek UIDs/FUIDs, five-tuples, DNS, HTTP, file, and TLS identifiers; eCAR
actor/object/process/session/network properties; RFC5424/RFC3164 syslog
host/app/procid fields; web/proxy request fields; ASA message/connection IDs and
tuples; Snort rule IDs and tuples; and Bash history time/command fields.

Schema v2 also records `attribution_status`, `mapping_method`,
`causal_parentage_status`, and contributor arrays for physical records that
aggregate more than one logical event. Generation fails before publishing the
sidecar if a receipt is unattributed, duplicated, missing on disk, or no longer
matches its final byte range and SHA-256. Source finalizers that rewrite content
(including ASA connection-ID normalization) refresh receipts from the final
file while preserving provenance by record position. XML `<Event>`
blocks and multi-line Bash history entries each count as one record. Line-based
sources use one physical line per record, so a CRLF log-forging payload creates
separately addressable physical records. Exclude this instructor-only sidecar
from blind-test and analyst-review bundles.

Schema v3 adds a generator-side, physical-record `detectability_class` plus
`detectability_reason` and `required_anchor_types`:

- `direct`: the row exposes the malicious action's primary semantics, such as a
  command line, process image, file path, or exploit request;
- `association_required`: the row is attributable using public log fields, but
  must be reached through bounded correlation such as host + ProcessGuid,
  host + PID + image + lifetime, Zeek UID/FUID, five-tuple + time, DNS answer,
  proxy client/target, eCAR actor/process ID, or ASA connection ID;
- `provenance_only`: generation provenance proves the row belongs to the
  malicious canonical event, but the physical row has no reliable public
  anchor with which a detector can recover that mapping;
- `not_applicable`: benign and red-herring rows.

The class is derived after final rendering because detectability depends on the
actual source row, not only the canonical `SecurityEvent`. A generator can set
an instructor-only canonical override for exceptional semantics; it is carried
out-of-band and is never rendered into source logs. Evaluation keeps all-record
F1 as the primary score and reports `direct + association_required` recall as a
separate diagnostic slice. All three v3 fields are private Ground Truth and are
removed from blind `RECORD_INDEX.jsonl` files.

For the exact `dataset_id` / `storyline_id` / `logical_event_id` /
`physical_record_id` generation formulas, uniqueness boundaries, stability
rules, and cross-team usage contract, see
[Record Identity and ID Generation](RECORD_IDENTITY_AND_ID_GENERATION.md).
...
| REGISTRY | READ, MODIFY | Windows 注册表操作。`READ` 是 eCAR 原生读取证据；读取不会伪造成 Sysmon 12/13 写入事件。 |

...
├── registry: RegistryContext (key, value, operation; read/modify 语义分离)
...
报告导入场景对注册表读取使用规范事件 `registry_read`，只交由原生支持的 eCAR
渲染为 `REGISTRY/READ`。Sysmon 12/13 仍只处理 `registry_modify`，因此不会为了增加日志条数
而改变原报告语义。场景事件可选的 `source_metadata` 会继续传入 Ground Truth schema v2，
用于披露事实编号、原报告位置、字段来源与预期可见性。

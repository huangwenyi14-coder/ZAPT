...
This detector is white-box with respect to framework generation mechanics, but
blind to scenarios and ground truth. It first finds EvidenceForge's finite
sleep+jitter beacon shape, then pivots on the same generated host/time window
for framework-correlated process, connection, scheduled-task, and logon traces.
"""
...
def main() -> int:
    ...
    if args.input:
        ...
        payload = {
            "cases": [
                {
                    "case_id": data_dir.parent.name or "record-case",
    ...
    if args.record_index:
        ...
        output = record_level_predictions(
            data_dir=case_data_dir,
            record_index=load_record_index(Path(args.record_index), data_dir=case_data_dir),
            findings=findings,
        )
    else:
